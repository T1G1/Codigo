...
CREATE TABLE IF NOT EXISTS `t1g1`.`loja` (
  ...
  `nic` INT(9) NOT NULL COMMENT 'Número de contríbuinte da loja.  ',
...
CREATE TABLE IF NOT EXISTS `t1g1`.`utilizadores` (
  ...
  INDEX `fk_utilizadores_tipo_utilizador1_idx` (`idtipo` ASC),
  ...
  CONSTRAINT `fk_utilizadores_tipo_utilizador1`
    FOREIGN KEY (`idtipo`)
    REFERENCES `t1g1`.`tipo_utilizador` (`idtipo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `t1g1`.`clientes` (
  ...
  `userName` VARCHAR(50) NOT NULL COMMENT 'User Name do cliente. Permite a associar os clientes aos utilizadores.',
  ...
  `emailCliente` VARCHAR(50) NULL COMMENT 'Endereço email do cliente.',
  `nifCliente` INT(9) NOT NULL COMMENT 'Número de contríbuinte do cliente. ',
  `moradaCliente` VARCHAR(100) NULL COMMENT 'Morada do cliente.',
  `codpostal` VARCHAR(8) NOT NULL,
  `tipoCliente` VARCHAR(4) NULL COMMENT 'Identificação do tipo de cliente. Particular ou empresa.',
  ...
  UNIQUE INDEX `nifCliente_UNIQUE` (`nifCliente` ASC),
  INDEX `fk_clientes_utilizadores_idx` (`userName` ASC),
  UNIQUE INDEX `nCartaoCliente_UNIQUE` (`nCartaoCliente` ASC),
  INDEX `fk_clientes_codigos_postais1_idx` (`codpostal` ASC),
  CONSTRAINT `fk_clientes_utilizadores`
    FOREIGN KEY (`userName`)
    REFERENCES `t1g1`.`utilizadores` (`userName`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `t1g1`.`funcionarios` (
  ...
  `userName` VARCHAR(50) NOT NULL COMMENT 'User name do funcionário.',
  ...
  INDEX `fk_funcionarios_utilizadores1_idx` (`userName` ASC),
...
CREATE TABLE IF NOT EXISTS `t1g1`.`tipopagamentos` (
  `idTipoPagamento` VARCHAR(2) NOT NULL COMMENT 'Identifica cada tipo de pagamento disponíveis para o cliente escolher. É chave primária desta tabela.\n',
  `DescrTipoPagamento` VARCHAR(100) NULL COMMENT 'Destcreve o tipo de pagamento associado ao respectivo código.',
  PRIMARY KEY (`idTipoPagamento`),
  UNIQUE INDEX `idTipoPagamento_UNIQUE` (`idTipoPagamento` ASC))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `t1g1`.`taxaivaproduto` (
  `idTaxa` VARCHAR(10) NOT NULL COMMENT 'Identificação simplificada da taxa de iva do produto.\nÉ chave primária e permite ligar à tabela produtos.',
...
CREATE TABLE IF NOT EXISTS `t1g1`.`tiposEntrega` (
  `idTipoEnt` VARCHAR(3) NOT NULL COMMENT 'Identifica do tipo de entraga. É chave primária da tabela.',
  `descrTipoEnt` VARCHAR(60) NULL COMMENT 'Descreve o tipo de pagamento associado ao respectivo código.\n',
  `custo` FLOAT NOT NULL DEFAULT 0 COMMENT 'Representa o custo associado ao serviço de entrega associado ao respectivo código. ',
  `idTaxa` VARCHAR(10) NOT NULL COMMENT 'Taxa de iva no serviço de entrega à data da confirmação da encomenda.',
  PRIMARY KEY (`idTipoEnt`),
  UNIQUE INDEX `idTipoEnt_UNIQUE` (`idTipoEnt` ASC),
  INDEX `fk_tiposEntrega_taxaivaproduto1_idx` (`idTaxa` ASC),
  CONSTRAINT `fk_tiposEntrega_taxaivaproduto1`
    FOREIGN KEY (`idTaxa`)
...
CREATE TABLE IF NOT EXISTS `t1g1`.`encomendas` (
  ...
  `idTipoEnt` VARCHAR(3) NOT NULL COMMENT 'Identificação do tipo de entrega. É chave estrangeira que permite a ligação à tabela de tipos de pagamento.',
  ...
  `idTipoPagamento` VARCHAR(2) NOT NULL DEFAULT 'MT' COMMENT 'Tipo de pagamento escolhido pelo cliente, segundo os tipos de pagamento definidos na tabela de tipos de pagamento.',
  ...
  INDEX `fk_encomendas_Tipopagamentos1_idx` (`idTipoPagamento` ASC),
  INDEX `fk_encomendas_clientes1_idx` (`idCliente` ASC),
  INDEX `fk_encomendas_tiposEntrega1_idx` (`idTipoEnt` ASC),
  INDEX `fk_encomendas_codigos_postais1_idx` (`codpostal` ASC),
  UNIQUE INDEX `codPagamento_UNIQUE` (`codPagamento` ASC),
  UNIQUE INDEX `codDespacho_UNIQUE` (`codDespacho` ASC),
  CONSTRAINT `fk_encomendas_Tipopagamentos1`
    FOREIGN KEY (`idTipoPagamento`)
    REFERENCES `t1g1`.`tipopagamentos` (`idTipoPagamento`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_encomendas_clientes1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_encomendas_tiposEntrega1`
    FOREIGN KEY (`idTipoEnt`)
    REFERENCES `t1g1`.`tiposEntrega` (`idTipoEnt`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `t1g1`.`marcas` (
  ...
  `codpostal` VARCHAR(8) NOT NULL COMMENT 'Código postal do Representante.',
  PRIMARY KEY (`idMarca`),
  UNIQUE INDEX `idMarca_UNIQUE` (`idMarca` ASC),
  UNIQUE INDEX `desigMarca_UNIQUE` (`desigMarca` ASC),
...
CREATE TABLE IF NOT EXISTS `t1g1`.`prateleira` (
  ...
  `desgPrateleira` VARCHAR(15) NOT NULL DEFAULT '\"Prateleira\"',
...
CREATE TABLE IF NOT EXISTS `t1g1`.`produtos` (
  ...
  `idMarca` INT NOT NULL COMMENT 'Chave estrangeira que permite a ligação com a tabela \"marcas\", associando a marca ao produto.',
  ...
  `imagemPesquisa` VARCHAR(120) NULL DEFAULT 'default' COMMENT 'Imagem do produto utilizada para apresentação nas pesquisas de produtos.\n\nQuando não é inserida imagem é apresentada a imagem por defeito.\n ',
  `idPrateleira` INT NOT NULL COMMENT 'Id da preteleira onde se encontra o produto. \nÉ chave estrangeira que permite ligar a tabela de produtos com a tabela de prateleiras.',
  `idCorredor` INT NOT NULL COMMENT 'Id do corredor  onde se encontra a preteleira do produto. \nÉ chave estrangeira que permite ligar a tabela de produtos com a tabela de prateleiras.',
  PRIMARY KEY (`idProduto`),
  UNIQUE INDEX `idProduto_UNIQUE` (`idProduto` ASC),
  INDEX `fk_Produtos_Marcas1_idx` (`idMarca` ASC),
  INDEX `fk_Produtos_Prateleira1_idx` (`idPrateleira` ASC, `idCorredor` ASC),
  ...
  CONSTRAINT `fk_Produtos_Prateleira1`
    FOREIGN KEY (`idPrateleira` , `idCorredor`)
    REFERENCES `t1g1`.`prateleira` (`idPrateleira` , `idCorredor`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `t1g1`.`detencomenda` (
  `item` INT NOT NULL AUTO_INCREMENT COMMENT 'Identifica a ordem dos vários produtos relativos a uma encomenda. é chave primária desta tabela conjuntamente com o idEncomenda.  ',
  ...
  `precoVenda` FLOAT NOT NULL DEFAULT 0 COMMENT 'Preço total do item da  encomenda, com o preço do produto há data da encomenda. Preço unitário x quantidade + iva. \n',
  `tIva` FLOAT NULL DEFAULT 0 COMMENT 'Representa o valor da taxa de iva à data da encomenda, do respectivo item.',
  PRIMARY KEY (`item`),
...
CREATE TABLE IF NOT EXISTS `t1g1`.`promocoes` (
  ...
  `desigPromocao` VARCHAR(25) NOT NULL COMMENT 'Designação da Promoção.\n',
  `descrPromocao` VARCHAR(60) NULL COMMENT 'Descrição da Promoção.\n',
  ...
  `idProduto` INT(10) NOT NULL COMMENT 'Código identificativo do produto em promoção. Permite a ligação à tabela de produtos.',
...
CREATE TABLE IF NOT EXISTS `t1g1`.`estadosEncomendas` (
  `idEstado` VARCHAR(3) NOT NULL COMMENT 'Identificação de cada estado que a encomenta pode ter.\nÉ chave primária desta tabela.',
...
CREATE TABLE IF NOT EXISTS `t1g1`.`estado_encomenda_data` (
  ...
  `codFuncionario` INT NOT NULL COMMENT 'Indica o código do funcionário verifica ou insere o estado da encomenda. ',
  `idEncomenda` INT NOT NULL COMMENT 'Identifica a encomenda associada. Fáz a ligação à tabela encomendas.\n',
  `idEstado` VARCHAR(3) NOT NULL COMMENT 'Identifica o estado a que passa a encomenda no momento em que é preenchida esta tabela. É conjuntamente com o idEncomenda a chave primária desta tabela. ',
  PRIMARY KEY (`idEncomenda`, `idEstado`),
  INDEX `fk_estado_encomenda_data_encomendas1_idx` (`idEncomenda` ASC),
  INDEX `fk_estado_encomenda_data_Estados1_idx` (`idEstado` ASC),
  CONSTRAINT `fk_estado_encomenda_data_encomendas1`
    FOREIGN KEY (`idEncomenda`)
    REFERENCES `t1g1`.`encomendas` (`idEncomenda`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_estado_encomenda_data_Estados1`
...
CREATE TABLE IF NOT EXISTS `t1g1`.`avalcoment` (
  `idavaliacao` INT NOT NULL AUTO_INCREMENT COMMENT 'Código identificativo da avaliação e comentário. É Chave Primária da tabela e é auto incremental. ',
  ...
  `codFuncionario` INT NOT NULL COMMENT 'Código do funcionário responsável pela validação dos comentários.',
  `obsvalid` VARCHAR(200) NULL COMMENT 'Introdução de comentários que levaram à não validação do comentário.',
  `datavalidacao` DATETIME NULL COMMENT 'Data da validação da avaliação e comentário por um funcionário da loja.',
  PRIMARY KEY (`idavaliacao`),
  UNIQUE INDEX `idavalcoment_UNIQUE` (`idavaliacao` ASC),
...
CREATE TABLE IF NOT EXISTS `t1g1`.`depcontactos` (
  `iddep` INT NOT NULL COMMENT 'Código identificativo de cada departamento da loja.',
  ...
  `desigTipoContacto` VARCHAR(30) NULL COMMENT 'Designação do Departamento.',
  ...
  PRIMARY KEY (`iddep`, `idloja`),
  UNIQUE INDEX `desigTipoContacto_UNIQUE` (`desigTipoContacto` ASC),
...
CREATE TABLE IF NOT EXISTS `t1g1`.`tipofoto` (
  `idtipo` INT NOT NULL DEFAULT 1 COMMENT 'É Chave Primária e Autoincrementavel. ',
...
CREATE TABLE IF NOT EXISTS `t1g1`.`fotosloja` (
  ...
  `idtipo` INT NOT NULL COMMENT 'Identifica o tipo da foto. Liga à tabela de tipofoto.',
  PRIMARY KEY (`idFoto`),
  INDEX `fk_fotosloja_loja1_idx` (`idloja` ASC),
  INDEX `fk_fotosloja_tipofoto1_idx` (`idtipo` ASC),
  ...
  CONSTRAINT `fk_fotosloja_tipofoto1`
    FOREIGN KEY (`idtipo`)
    REFERENCES `t1g1`.`tipofoto` (`idtipo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
